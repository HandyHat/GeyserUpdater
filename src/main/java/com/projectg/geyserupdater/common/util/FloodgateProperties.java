...
public class FloodgateProperties {
    ...
    public static boolean isLatestBuild() throws IOException {
        int jenkinsBuildNumber = getLatestFloodgateBuildNumberFromJenkins(getFloodgateGitPropertiesValue("git.branch"));
        int localBuildNumber = Integer.parseInt(getFloodgateGitPropertiesValue("git.build.number"));
     ...
     */
    public static String getFloodgateGitPropertiesValue(String propertyKey) throws IOException {
        Properties gitProperties = new Properties();
        gitProperties.load(Bukkit.getPluginManager().getPlugin("floodgate").getResource("git.properties"));
     ...
     */
    public static int getLatestFloodgateBuildNumberFromJenkins(String gitBranch) throws UnsupportedEncodingException {
        String buildXMLContents = WebUtils.getBody("https://ci.opencollab.dev/job/GeyserMC/job/Floodgate/job/" + URLEncoder.encode(gitBranch, StandardCharsets.UTF_8.toString()) + "/lastSuccessfulBuild/api/xml?xpath=//buildNumber");
